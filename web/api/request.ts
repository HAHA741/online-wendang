...
// 1. 定义默认的基础地址
let baseURL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:38222';

// 2. 只有在浏览器环境下，才动态获取 host
if (typeof window !== 'undefined') {
  const host = window.location.hostname;
  const httpProtocol = window.location.protocol;
  baseURL = process.env.NEXT_PUBLIC_API_URL || `${httpProtocol}//${host}:38222`;
} else {
  // 3. 在服务端（Docker 内部）渲染时，使用内部服务名
  // 注意：如果你是在构建静态页面，这里建议指向后端容器名
  baseURL = 'http://fortune-backend:38222'; 
}
// 创建 axios 实例
